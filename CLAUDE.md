...
Page tree (admin-only): `yconverter` → `convert` → {`redaxo`, `yform`, `seo42`}, plus `settings` → {`general`, `ai`}. `index.php` dispatches via `rex_be_controller::includeCurrentPageSubPath()` (sub-path `a/b/c` → file `b.c.php`).

- **`convert.redaxo.php`** — the main wizard: clone → migrate (per package) → media (Steps 1–3), plus reset and the chunked AJAX runner. Links to the YForm/SEO42 tabs.
- **`convert.yform.php`** — the custom-table → YForm mapping wizard (analyze → preview → apply); self-contained.
- **`convert.seo42.php`** — the seo42 URL-control → `url` addon migration wizard; self-contained.
- **`settings.general.php`** / **`settings.ai.php`** — settings sub-pages (source DB + media; AI). Config I/O centralized in `Config` (`read()`/`write()`), each sub-page overlays only its own fields.
...
`YFormImporter` consumes confirmed `FieldMapping[]`: `analyze()` (detect, write nothing), `import()` (new staging table → YForm table), `refreshFields()` (re-detect an already-imported `rex_yf_*` table, replacing only its `yform_field` rows). **`convert.yform.php`** is the **analyze → preview → apply** wizard with two lists: new custom tables and already-imported YForm tables (retroactive re-detection via `detectExistingYFormTables()`). The console `yconverter:run --dry-run` prints the detected mappings for both lists and writes nothing (read-only — it short-circuits before clone/migrate).

**`FieldTypes`** is the addon-aware type catalogue: `all()` = the full list (`SchemaDetector::allowedTypes()`); `available()` filters it to types whose providing addon is installed (`mform` → custom_link*/imagelist/color_swatch/medialist/linklist; `yform_lang_fields` → lang_*; everything else is YForm core, incl. `be_user`/`be_link`/`email`). The preview dropdown and the AI catalogue use `available()`; the detector downgrades a rule whose target type isn't available to the column-type fallback. Detection rules cover the safe/core cases (status, url→text+type=url, be_media, be_user, email→email, color_swatch when mform present, year/price/textarea, i18n); the rest are operator-selectable.
