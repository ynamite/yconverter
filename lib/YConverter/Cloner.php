...
class Cloner
{
    ...
    public function fetchTables()
    {
        ...
        foreach ($this->getSourceTables() as $table) {
            try {
                ...
                $tablesFailed[] = $table.': '.$e->getMessage();
            }
        }
        ...
        if (\count($tablesFailed)) {
            $this->message->addError(sprintf('Folgende Tabellen konnten nicht geklont werden:<br /><br /><pre class="rex-code">%s</pre>', implode('<br />', $tablesFailed)));
        }
    }

    /**
     * Source (REDAXO 4) tables to clone, with their original prefix.
     *
     * @return string[]
     */
    ...
    /**
     * Clones a single source table (structure + data) into the `yconverter_*` staging
     * table. Throws rex_sql_exception on failure.
     */
    public function cloneTable($table)
    {
        $sqlOutdated = \rex_sql::factory($this->config->getOutdatedDatabaseId());
        $insertSize = 4000;
        ...
        $this->sql->setQuery('DROP TABLE IF EXISTS '.$this->sql->escapeIdentifier($tableClone));
    ...
    public function dropTables()
    {
        ...
        if (!isset($dbConfig['1']['name'])) {
            $this->message->addError('Die REDAXO-5-Datenbank ist nicht korrekt konfiguriert.');
            return;
        }
